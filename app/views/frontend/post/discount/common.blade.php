...
<div class="sheet">
    <div class="form-group input-group">
        ...
        <select name="benefits_id" id="benefits_id" class="form-control">
            <option value="" disabled>{{ trans('messages.please-select-discount-type') }}</option>
            <option value="{{ URL::to( Category::findBySlug('discount')->getSlug() ) }}">
                {{ trans('messages.all') }}
            </option>

            @foreach( Category::findBySlug('discount')->subCategories() as $subcategory )
            <optgroup class="input-theme" label="{{ $subcategory->translate()->name }}">
                @foreach( $subcategory->subCategories() as $subsubcategory )
                <option value="{{ URL::to($subsubcategory->getSlug()) }}"{{ ($frontend_current_category->id == $subsubcategory->id) ? ' selected' : '' }}>-- {{ $subsubcategory->translate()->name }}</option>
                @endforeach
            </optgroup>
            @endforeach
    ...
    @if( count($frontend_current_category->getPosts()) > 0 )
