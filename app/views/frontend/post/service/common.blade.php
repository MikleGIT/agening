...
<div class="list">
    ...
    <div class="article">
        <div class="post">
            <h3 class="post-title text-center">{{ $frontend_current_category->translate()->name }}</h3>
            <div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">

                @foreach( $frontend_current_category->getPosts() as $i => $post )
                @if($post->translate()->title != '')
                <div class="panel panel-default">
                    <div class="panel-heading" role="tab" id="headingOne">
                        <h4 class="panel-title">
                            <a role="button" data-toggle="collapse" data-parent="#accordion--" href="#collapse-{{ $post->id }}" aria-expanded="<?php /*{{ ($i == 0) ? 'true' : 'false' }}*/ ?>" aria-controls="collapse-{{ $post->id }}">
                                {{ $post->translate()->title }}
                            </a>
                          </h4>
                    </div>
                    <div id="collapse-{{ $post->id }}" class="panel-collapse collapse<?php /*{{ ($i == 0) ? ' in' : '' }}*/ ?>" role="tabpanel" aria-labelledby="heading-{{ $post->id }}">
